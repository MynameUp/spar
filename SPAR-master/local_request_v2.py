...
class ModelConfig:
    ...
    is_api: bool = False  # True for cloud API (百炼/OpenAI), False for local vLLM/SGLang
...
MODEL_CONFIGS = {
    ...
    "qwen3.5-plus": ModelConfig(
        ...
        openai_client=OpenAI(
            api_key=os.getenv("DASHSCOPE_API_KEY", "sk-ws-H.EIIIHRX.gw84.MEYCIQCoP6lcCqplvS5ZiKDuGrQ2pATHPBnqA6HT5twUYPNlvAIhAKs7HggpRmFJ1W1AW8K4T5orox2rZCQe1mI4CW5yjcPe"),
...
class LLMClient:
    ...
    def _make_request(
        ...
        try:
            config = MODEL_CONFIGS.get(data["model"])
            if config and config.is_api:
    ...
    def _make_api_request(self, data: Dict[str, Any]) -> Optional[str]:
        """Handle requests for cloud API models (百炼/OpenAI) using OpenAI client"""
        try:
            ...
            logger.error(
                f"API request failed for {data['model']}: {str(e)}--{traceback.format_exc()}"
            )
            return None
...
def get_from_llm(
    messages: Union[str, List[Dict[str, str]]], model_name: str = "qwen3.5-plus", **kwargs
) -> Optional[str]:
    """
