...
_serper_last_request = 0.0
SERPER_MIN_INTERVAL = 2.0  # 免费账户建议≥2秒
...
def fetch_reference_openalex(ref_id):
    ...
    try:
        ...
        ref_response = requests.get(ref_url, params={"api_key": OPENALEX_API_KEY})
...
def search_doc_via_url_from_openalex(data):
    ...
    for ref_id in referenced_works:
        try:
            ref_url = f"https://api.openalex.org/works/{ref_id}"
            ref_response = requests.get(ref_url, timeout=10, params={"api_key": OPENALEX_API_KEY})
            if ref_response.status_code == 200:
                ref_data = ref_response.json()
                is_open = ref_data.get("open_access", {}).get("is_oa", False)
                if not is_open:
...
def search_paper_via_query_from_openalex(
    ...
    try:
        ...
        params = {
            "search": keyword.replace("|", " OR "),  # 短语搜索,|转OR
            "sort": "relevance_score:desc",  # 相关性排序而非引用量
            "filter": f"open_access.is_oa:true,publication_year:>{start_year}",
            # "search": keyword,  # 全文搜索
                ...
                for work in data["results"]:
                    try:
                        ...
                        if abstract_inverted_index is not None:
                            ...
                            abstract = " ".join(
                                w for _, w in sorted(word_positions)
                            )
                        else:
                            abstract = ""
...
def search_paper_via_query_from_semantic(query, max_paper_num=15, end_date=None):
    ...
    from global_config import SEMANTIC_SCHOLAR_API_KEY
...
def google_search_arxiv_id(query, try_num=4, num=10, end_date=""):
    ...
    # 免费账户限制：长查询触发 "Query pattern not allowed"
    ...
    global _serper_last_request
    for _ in range(try_num):
        try:
            with _serper_lock:
                wait = SERPER_MIN_INTERVAL - (time.time() - _serper_last_request)
                if wait > 0:
                    time.sleep(wait)
                try:
                    ...
                    _serper_last_request = time.time()
            # response = requests.request(
            #     "POST", url, headers=headers, data=payload, timeout=10, proxies=PROXIES
            # )
            if response.status_code == 200:
                ...
                if response.status_code == 400:
                    time.sleep(2 ** (_ + 1))  # 指数退避: 2s,4s,8s,16s
...
def get_openalex_info(arxiv_id):
    ...
    response = requests.get(url, proxies=PROXIES, params={"api_key": OPENALEX_API_KEY})
    if response.status_code == 200:
        data = response.json()
        return {
